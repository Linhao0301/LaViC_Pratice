        ...
        if os.path.exists(image_path):
            try:
                ...
                if image.size[0] == 0 or image.size[1] == 0:
                    print(f"[WARNING] Invalid image size for {image_path}, using default image")
                    image = self.default_image
            except (UnidentifiedImageError, OSError, IOError, Exception) as e:
                print(f"[WARNING] Failed to load image {image_path}: {str(e)}, using default image")
                image = self.default_image
        else:
...
class DataCollator:
    ...
    def __call__(self, batch):
        ...
        # Build prompt-target pairs with error handling
        for item in batch:
            try:
                title = item.get("title", "No Title")
                desc = item.get("description", "No Description")
                
                # 确保title和description不为空
                if not title.strip():
                    title = "No Title"
                if not desc.strip():
                    desc = "No Description"
                ...
                print(f"[WARNING] Error processing batch item: {str(e)}, skipping item")
                # 跳过有问题的item，继续处理其他items
                continue
...
class PretrainVisionModel(pl.LightningModule):
    ...
    def forward(self, input_ids, attention_mask, images, image_token_mask, labels=None):
        ...
        # Use full model with custom inputs_embeds instead of just language_model
        outputs = self.model(
...
def main():
    ...
    parser.add_argument("--max_samples", type=int, default=None, help="Maximum number of training samples (for quick testing)")
